...
function updatePlant(id, plant) {
  return db("plants").where("id", Number(id)).update(plant);
}

async function addWatering(plant_id, watering) {
  const [id] = await db("plants")
    .insert({ plant_id, watering_time: watering.watering_time })
    .returning("id");
  return findById(id);
}

function getWateringSchedule(plant_id) {
  return db("watering").where({ plant_id }).select("id", "watering_time");
}
function deleteWateringbyId(plant_id) {
  return db("watering").where({ plant_id }).delete();
}
function deleteWateringTime(id) {
  return db("watering").where({ id }).delete();
}
